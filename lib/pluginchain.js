...
function createChainer(router){
  ...
  function doChain(route){
    ...
    sys.puts(str);
    sys.puts(sys.inspect( router.modulePrototypesContainer));
    return function(){
      var args = Array.prototype.slice.call(arguments);   
      
      //var scope = Object.create(router.helperContainer);
      var scope = router.createScope(args[0], args[1]);

      // insert the request ans response in all 
      ...
      function chain(actions) {
        ...
        scope.next = function next() {
          ...
          pos++;
          if (pos < length) {
            actions[pos].apply(scope, [].concat( args, scope));
          } else {
          }
        } // end next()
