...
function createChainer(router){
  ...
  function doChain(route){
    ...
    return function(){
      ...
      //sys.puts(typeof )
      //var hlpContainer  = new router.helperContainer(args[0], args[1]);
      var hlpContainer = Object.create(router.helperContainer);
      router.insertScopeObject(hlpContainer, args[0], args[1]);
      //sys.puts(sys.inspect(hlpContainer));    
      ...
      function chain(actions) {
        ...
        ndt.next = function next() {
          ...
          if (pos < length) {
            ...
            //actions[pos].apply(route.helpers, [].concat( args, ndt));
            actions[pos].apply(hlpContainer, [].concat( args, ndt));
          } else {
