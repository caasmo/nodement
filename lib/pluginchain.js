...
function createChainer(router){
  ...
  // rename chains 
  // recibo nodement object y lo paso
  // no necesito objeto.
  // necesito pasar un objeto con propiedades
  // y que a la vez sirva de almacenamiento. 
  // almacenamiento para la cadena?
  // ndt es global al server.
  // necesito objeto attached to chain -> redirect?
  var ndt =  router;
  ...
  function doChain(route, errChain){
    ...
    return function(){
      ...
      //sys.puts(JSON.stringify(route.plugins.post[1].name + 'hppp'));
          ...
          if(argums[0] instanceof Error){
            ...
            if (index !== -1){
               ...
               if (rte === 'this'){
                 ...
                 // route exists?
                 /* 
                 var theRoute = router.routes[rte]; 
                 if (theRoute){
                    // create chain
