...
exports.createControllerChain = createControllerChain;
...
function errorChainMain(obj1, obj2){
...
// 
function createControllerChain(func, collector){
  sys.puts("controller generator called");
    ...
    function chain(actions, callback) {
      ...
      function next() {
        ...
        if(argums[0] instanceof Error){
          ...
          var errCh = createControllerChain(errorChainMain);//sync
          ...
          var fn = createControllerChain(argums[0]);//sync
        ...
        pos++;
        if (pos >= length) {
          //callback.apply(helpers, [].concat(null, args));
          //callback.apply(null, [].concat(null, args));
        } else {
          actions[pos].apply(helpers, [].concat( args, next));
          //actions[pos].apply(null, [].concat( args, next));
        }
      }
