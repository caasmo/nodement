...
exports.nodement = function() {
  ...
  var server = http.createServer(function (req, res) {
    ...
    var ndt = response.create(router, req, res); 
    
    ndt.id = Math.floor(Math.random() * 10000);

    var route = router.doRoute(req, ndt);
    chRouter.doChain(route)( ndt);
  }); 
...
function addStandardHeaders( ndt){
    len = ndt.body.length;
    ndt.headers["Content-Length"] = len;  
    ndt.headers["Content-Type"] = "text/plain";
    ndt.next();  
}
